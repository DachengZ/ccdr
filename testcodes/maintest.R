source("rmvDAG.R") ## generate random data with points allowed to be fixed (in a specified range)
source("convert.R")
source("compare.R") ## compare graphs. different from `compareGraphs`

maintest <- function(g, vfix = NULL, nn = NULL, N = 50, gamma = 2, lambdas.length = 20, originaldata = NULL, originalvfix = NULL) {
    ## g: input DAG
    ...
    rownames(edges) <- colnames(edges) <- 1:pp
    time <- rep(0, N)
    ...
    for(testi in 1:N) {
        ...
        ### Run the algorithm
        ccdr.path <- ccdr.run(data = dat1, intervention = vfix1, gamma = gamma, lambdas.length = lambdas.length, alpha = 10, verbose = FALSE)
...
#         }
    ...
    return(list(edges = edges, metric = metric, time = sum(time), samples = nn, tests = N, data = dat, vfix = c(vfix, originalvfix)))
}
...
redge <- function(test) {
    ## determine reversedly estimated edges
    edges <- test$edges
...
tedge <- function(test) {
    ## determine correctly estimated edges
    edges <- test$edges
